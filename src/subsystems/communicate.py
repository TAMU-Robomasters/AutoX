# ruff: noqa
# mypy: ignore-errors
# TODO: refactor this subsystem to handle multiple processes wanting embedded data, or wanting to send data to embedded.
        ...
        try:
            ...
            subprocess.run(
                [
                    "bash",
                    "-c",
                    f'sudo -S chmod 777 \'{serial_port}\' <<<  "$(cat "$HOME/.pass")" ',
                ]
            )
...
def when_aiming_refreshes(panels, start_time):
    ...
    if config.debug:
        print(
            f"""msg({f"X:{message_to_embedded.X:.4f}".rjust(7)}, {f"Y:{message_to_embedded.Y:.4f}".rjust(7)}, {f"Z:{message_to_embedded.Z:.4f}".rjust(7)}, {f"delay:{message_to_embedded.capture_delay}"}ms")"""
        )
