...
# TODO: add this to display subsystem
import cv2 as cv
import numpy as np
...
def draw(panel, frame):
    ...
    cv.putText(
        ...
        (
            "tvec: "
            + str(int(panel.tvec[0]))
            + " "
            + str(int(-1 * panel.tvec[1]))
            + " "
            + str(int(panel.tvec[2]))
        ),
    ...
    cv.putText(
        ...
        (
            "rvec: "
            + str(int(np.degrees(panel.rvec[0])))
            + " "
            + str(int(np.degrees(panel.rvec[1])))
            + " "
            + str(int(np.degrees(panel.rvec[2])))
        ),
    ...
    dist, cam_matrix = video_stream.get_intrinsics()
