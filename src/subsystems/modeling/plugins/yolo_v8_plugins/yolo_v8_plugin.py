...
class Yolov8(object):
    ...
    def __init__(self, model_filepath, input_dimension, acceleration, warmup=True):
        ...
        if self.acceleration in [None, "cpu", "gpu"]:
            ...
            self.nms = lambda preds, *args, **kwargs: non_max_suppression(
                preds, *args, **kwargs
            )[0]
            ...
            print(
                "[modeling]   Warning: TensorRT will use prebuilt NMS value of 0.2. To change, rebuild TensorRT engine with new value.\n"
            )
        else:
    ...
    def preprocess_image(self, image_pre):
        ...
        image_tensor = torch.as_tensor(
            image_pre, device=self.device
        )  # create torch tensor directly on device (GPU or CPU)
        image_tensor = image_tensor.permute(2, 0, 1)  # HWC to CHW
        image_tensor = image_tensor[[2, 1, 0]].unsqueeze(
            0
        )  # RGB to BGR, add batch dimension
        image_tensor = F.interpolate(
            image_tensor, size=(self.input_w, self.input_h)
        ).div(255.0)  # resize, normalize from 0-255 to 0-1.0
    ...
    def postprocess_preds(
        self, yolo_tens, image_post_h, image_post_w, conf_thres, detect_class=False
    ):
        ...
        preds[:, :4] = self.rescale_coords_to_original(
            preds[:, :4], (image_post_h, image_post_w)
        ).round()
