...
try:
    ...
    raise SystemExit(
        f"ERROR: failed to load {path_to.tensorrt_so_file}  "
...
def add_yolo_plugins(network, model_name, num_classes, logger):
    ...
    if len(anchors) != len(yolo_whs):
        raise ValueError(
            "bad number of yolo layers: %d vs. %d" % (len(anchors), len(yolo_whs))
        )
    if network.num_outputs != len(anchors):
        raise ValueError(
            "bad number of network outputs: %d vs. %d"
            ...
            plugin_creator.create_plugin(
                ...
                trt.PluginFieldCollection(
                    [
                        trt.PluginField(
                            "yolo_width",
                            np.array(yolo_whs[i][0], dtype=np.int32),
                            trt.PluginFieldType.INT32,
                        ),
                        trt.PluginField(
                            "yolo_height",
                            np.array(yolo_whs[i][1], dtype=np.int32),
                            trt.PluginFieldType.INT32,
                        ),
                        trt.PluginField(
                            "input_multiplier",
                            np.array(input_multiplier, dtype=np.int32),
                            trt.PluginFieldType.INT32,
                        ),
                        trt.PluginField(
                            "new_coords",
                            np.array(new_coords, dtype=np.int32),
                            trt.PluginFieldType.INT32,
                        ),
                        trt.PluginField(
                            "num_classes",
                        ...
                        trt.PluginField(
                            "num_anchors",
                            np.array(len(anchors[i]) // 2, dtype=np.int32),
                            trt.PluginFieldType.INT32,
                        ),
                        trt.PluginField(
                            "anchors",
                            np.ascontiguousarray(anchors[i], dtype=np.float32),
                            trt.PluginFieldType.FLOAT32,
                        ),
                        trt.PluginField(
                            "scale_x_y",
                            np.array(scales[i], dtype=np.float32),
                            trt.PluginFieldType.FLOAT32,
                        ),
                    ]
                ),
