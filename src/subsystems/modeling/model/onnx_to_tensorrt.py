...
from plugins import add_yolo_plugins, get_input_wh
...
def load_onnx(model_name):
    ...
    if not os.path.isfile(onnx_path):
        print(
            "ERROR: file (%s) not found!  You might want to run yolo_to_onnx.py first to generate it."
            % onnx_path
        )
        return None
    else:
...
def build_engine(model_name, category_num, do_int8, dla_core, verbose=False):
    ...
    with (
        ...
        network = add_yolo_plugins(network, model_name, category_num, TRT_LOGGER)
