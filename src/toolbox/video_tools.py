# ruff: noqa
# mypy: ignore-errors
# TODO: figure out if we need this file
...
class Video(object):
    def __init__(self, path=None):
        ...
        def _cv_get(value):
            ...
            if video_capture.isOpened() == False:
                raise Exception(
                    f"Error, tried opening {self.path} with cv2 but wasn't able to"
                )
    ...
    def frames(self):
        ...
        if not FS.is_file(self.path):
            raise Exception(
                f"Error, tried opening {self.path} but I don't think that is a file"
            )
    ...
    def save_with_labels(self, list_of_labels, to=None):
        ...
        new_video = cv2.VideoWriter(
            output_file, cv2.VideoWriter_fourcc(*"mp4v"), fps, frame_dimensions
        )
    ...
    def save_clip(self, start, end, save_to):
        ...
        list_of_frames = []
        fps = self.fps()
        for each_frame, each_index, each_time in self.frames_with_time():  # ty:ignore[unresolved-attribute]
    ...
    def create_from_frames(self, list_of_frames, save_to=None, fps=30):
        ...
        if len(list_of_frames) == 0:
            raise Exception(
                "The Video.create_from_frames was given an empty list (no frames)\nso a video cannot be made"
            )
        elif save_to == None:
            raise Exception(
                "The Video.create_from_frames was given no save_to= location so it doesn't know where to save the file"
            )
        ...
        frame_dimensions = (frame_width, frame_height)
        new_video = cv2.VideoWriter(
            save_to, cv2.VideoWriter_fourcc(*"mp4v"), fps, frame_dimensions
        )
        # add all the frames
...
class VideoWriter:
    ...
    def save(self):
        if not self.is_finished and self.writer:
            print(
                f"""saving {self.number_of_pending_frames} frames to: {self.save_to}"""
            )
            # combine the resulting frames into a video, which will write to a file
            self.writer.release()
